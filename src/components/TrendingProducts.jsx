...
import ProductCard from './ProductCard'; // Reuse your styled ProductCard
...
export const getTrendingProducts = () => {
  const clickData = JSON.parse(localStorage.getItem('productClicks')) || {};
  return allProducts
    .map(product => ({
      ...product,
      clicks: clickData[product.id] || 0,
    }))
    .filter(product => product.clicks > 0)
    .sort((a, b) => b.clicks - a.clicks)
    .slice(0, 4); // Top 5 trending
};
...
const TrendingProducts = () => {
  ...
  return (
    <Container className="my-5">
      <h3 className="mb-4"> Our Trending Products!!</h3>
      <Row>
        {trendingProducts.length > 0 ? (
          trendingProducts.map(product => (
            <Col key={product.id} xs={12} sm={6} md={3} className="mb-4 d-flex justify-content-center">
              {/* Use the full ProductCard */}
